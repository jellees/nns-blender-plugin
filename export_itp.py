...
class NitroTXPData:
    ...
    def find_plt_img_frm(self, palettes, images, frames):
        """
        Entry other than the object :list of frames, list of images and
        list of palettes, all of the same length.
        role : finds an id in self.frame_ids, self.palette_ids and
        self.image_ids where the corresponding input list is equal
        to a portion of the former list starting at the id, if its
        not found it adds the input lists to their corresponding
        list and returns the next id after the last id before the addition.
        """
